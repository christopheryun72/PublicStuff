...
result = requests.get('https://www.law.cornell.edu/supct/justices/thomas.dec.html')
...
for url in cases[::10]:
	try:
		...
		opinion = sent[sent.index("Justice Thomas delivered the opinion of the Court.") + len("Justice Thomas delivered the opinion of the Court."): ]
...
thomas = ["Thomas"] * len(excelReady)

df = pd.DataFrame()
df['Sentence'] = excelReady[:]
df['Judge'] = thomas[:]

df.to_excel('chrisJudges.xlsx', index = False)
